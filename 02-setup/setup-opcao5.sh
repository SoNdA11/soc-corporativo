#!/bin/bash
# ============================================================
# SETUP AUTOMATIZADO — SOC Corporativo (Wazuh Docker + 1 VM)
# ============================================================
# Executar no HOST (Arch Linux) como usuário NORMAL.
# Alguns comandos usam sudo.
#
# Uso: bash setup-opcao5.sh
# ============================================================
...
# Override log prefix for this script
...
[ -f /usr/bin/docker ] || err "Docker não encontrado. Instale com: sudo pacman -S docker"
docker compose version >/dev/null 2>&1 || warn "Docker Compose plugin não encontrado. Instale com: sudo pacman -S docker-compose"
[ -f /usr/bin/vboxmanage ] || warn "VirtualBox não encontrado. Instale com: sudo pacman -S virtualbox"
[ -f /usr/bin/restic ] && log "Restic OK" || warn "Restic não encontrado. sudo pacman -S restic"
[ -f /usr/bin/git ] || err "Git não encontrado. sudo pacman -S git"

# Verificar se vboxnet0 existe
vboxmanage list hostonlyifs | grep -q "vboxnet0" || {
...
# O wazuh-docker deve estar como irmao de soc-corporativo
...
# Gerar certificados (só na primeira vez)
...
if vboxmanage list vms | grep -q "kali-attacker"; then
...
# Rede Docker
docker network inspect target-net >/dev/null 2>&1 || {
  docker network create --subnet=172.20.0.0/16 target-net
  log "Rede target-net criada"
}

# DVWA
if docker inspect dvwa >/dev/null 2>&1; then
...
# Restic
run_root pacman -S --noconfirm restic 2>/dev/null || warn "Instale restic manualmente: sudo pacman -S restic"

run_root mkdir -p /backup-repo
run_root chmod 777 /backup-repo

# Inicializar repositório (se vazio)
...
# Arquivo de senha do Restic (evita senha hardcoded nos scripts)
...
# Copiar dados restaurados para o diretorio original e limpar
...
# Adicionar chave publica ao authorized_keys do usuario atual
...
# Copiar chave privada e script de ataque para o Kali
